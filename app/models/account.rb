class Account < ActiveRecord::Base
  ...
  def yesterday_average_spend
    r = 0
    rep = month_report(Date.today)
    r = rep.previous_day_average if rep
    r
  end

  def day_average_change
    calc_change(day_average_spend,yesterday_average_spend)
  end
  ...
  def yesterday_spend
    yesterday_average_spend
  end

  def day_spend_change
    calc_change(day_spend,yesterday_spend)
  end

  def last_update
    month_report.updated_at
  end
